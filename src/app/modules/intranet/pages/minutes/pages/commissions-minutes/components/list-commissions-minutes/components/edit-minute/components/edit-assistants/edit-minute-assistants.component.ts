...
@Component({
  standalone: true,
  selector: 'app-edit-commission-minute-assistants',
  templateUrl: './edit-minute-assistants.component.html',
  imports: [ReactiveFormsModule, CompletePaginationComponent],
})
export class EditCommissionMinuteAssistantsComponent implements OnInit {
  ...
  public handleAddAssistant() {
    ...
    return this.commissionMinuteService
      .registerAssistant(this.minute.id, activeUserId!, !!attended)
      .subscribe({
        next: () => {
  ...
  public handleDeleteAssistant(id: string) {
    this.loadingOperation = true;
    return this.commissionMinuteService
      .removeAssistant(this.minute.id, id)
      .subscribe({
        next: () => {
          this.loadingOperation = false;
          this.toastService.success(
    ...
    this.searchDebounce
      .pipe(
        ...
        switchMap((searchTerm) => {
          ...
          return this.userService.fetchUsersByName(
            searchTerm,
            undefined,
            this.minute.commission.networkCommission,
          );
        }),
        switchMap((users) => {
          if (users.length === 0) {
          ...
          const statusObservables = users.map((user) =>
            this.commissionMinuteService
              .fetchAssistantStatusByUserId(this.minute.id, user.id)
              .pipe(
