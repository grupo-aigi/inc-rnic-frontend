...
import { GroupMinuteSearchRecommendation } from './minutes.interfaces';
...
export class GroupMinutesService {
  searchGroupMinutes(searchTerm: string): any {
    throw new Error('Method not implemented.');
  }
  private _baseUrl: string = `${environment.baseUrl}/minutes/groups`;

  public constructor(private readonly http: HttpClient) {}

  public async fetchMinutes(
    pagination: Pagination,
    minutesFilterCriteria?: GroupMinutesFilterCriteria,
  ): Promise<MinutesResponse<GroupMinuteInfo>> {
    ...
    return lastValueFrom(
      this.http.get<MinutesResponse<GroupMinuteInfo>>(this._baseUrl, {
        params,
        headers,
      }),
    );
  ...
  public createMinute(minute: GroupMinuteInfo): Observable<any> {
    const headers = new HttpHeaders({
      Authorization: `Bearer ${sessionStorage.getItem('access_token')}`,
  ...
  public fetchAllGroupMinutesSearchRecommendations(
    searchTerm: string,
    group: NetworkGroups,
  ) {
    const headers = new HttpHeaders({
    ...
    return this.http.get<GroupMinuteSearchRecommendation[]>(
      `${this._baseUrl}/recommendations?search=${searchTerm}&group=${group}`,
      { headers },
    );
  }

  public editMinute(id: string, minuteInfo: GroupMinuteInfo) {
    const headers = new HttpHeaders({
      Authorization: `Bearer ${sessionStorage.getItem('access_token')}`,
      'Content-Type': 'application/json',
    });
