...
import labels from './scientific-ecosystem-update.lang';

@Component({
  standalone: true,
  selector: 'app-scientific-ecosystem-update',
  templateUrl: './scientific-ecosystem-update.component.html',
  imports: [ReactiveFormsModule, UploadOrReuseImageComponent],
})
export class ScientificEcosystemUpdateComponent implements OnInit {
  @Input() public scientificEcosystemInfo: NCPInfo | undefined;
  @Input() public scientificEcosystemImage: string = '';
  ...
  public ngOnInit(): void {
    if (!this.scientificEcosystemInfo) return;
    this.formGroup.get('id')?.setValue(this.scientificEcosystemInfo.id);
    this.formGroup.get('title')?.setValue(this.scientificEcosystemInfo.title);
    this.formGroup
      .get('subtitle')
      ?.setValue(this.scientificEcosystemInfo.subtitle);
    this.paragraphs = this.scientificEcosystemInfo.paragraphs;
    this.formGroup
      .get('videoUrl')
      ?.setValue(
        `https://www.youtube.com/watch?v=${this.scientificEcosystemInfo.videoUrl}`,
      );
    this.formGroup.get('email')?.setValue(this.scientificEcosystemInfo.email);
    this.links = this.scientificEcosystemInfo.links;
    this.formGroup
      .get('imageName')
      ?.setValue(this.scientificEcosystemInfo.imageName);
  }
