...
import labels from './create-resource-content-type2.lang';

@Component({
  standalone: true,
  selector: 'app-create-resource-content-type2',
  templateUrl: './create-resource-content-type2.component.html',
  imports: [ReactiveFormsModule],
})
export class CreateResourceContentType2Component {
  @Input() public target!: ContentTarget;
  @Output() public onSubmit: EventEmitter<ResourceContentType2> =
    new EventEmitter();
  ...
  public handleSubmit() {
    ...
    const subtitle = this.formGroup.get('subtitle')?.value as string;

    this.onSubmit.emit({
      title,
      subtitle,
      paragraphs: this.paragraphs,
      TYPE: 'CONTENT__TITLE_SUBTITLE_AND_PARAGRAPH',
    });
    this.paragraphs = [];
