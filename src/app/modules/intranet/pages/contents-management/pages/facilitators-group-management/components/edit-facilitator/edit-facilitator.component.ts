...
import labels from './edit-facilitator.labels';

@Component({
  standalone: true,
  templateUrl: './edit-facilitator.component.html',
  imports: [ReactiveFormsModule, UploadOrReuseImageComponent],
})
export class EditFacilitatorPage implements OnInit {
  ...
  public ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      if (!id) {
        ...
        //   '/intranet/gestion-contenido/grupo-facilitador',
        // ]);
        return;
      }
      this.networkParticipantId = +id;
      this.partnersService
        .fetchNetworkParticipantById('FACILITATOR', +id)
        .subscribe({
          next: (value) => {
          ...
          error: (err) => {
            this.toastrService.error(
              labels.errorLoadingFacilitatorInfo[this.lang],
            );
  ...
  public handleAddRole() {
    ...
    if (!roleText?.value) {
      this.toastrService.error(labels.roleRequired[this.lang]);
      return;
    }
    if (this.editMode) {
      ...
      .subscribe({
        next: () => {
          this.toastrService.success(
            labels.facilitatorUpdatedSuccessfully[this.lang],
          );
          return this.router.navigate([
            '/intranet/gestion-contenido/grupo-facilitador',
          ]);
        ...
        error: (err) => {
          this.toastrService.error(
            'Ha ocurrido un error al actualizar el facilitador',
          );
        },
      });
  }
